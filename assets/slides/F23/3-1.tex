...
\begin{itemize}
\item we got a good start on this material back in \S1.3; it is not really new \dots
\item this section \S3.1 requires ability to solve these types of DEs:
    \begin{enumerate}
    \item the easiest DE: $y'=ky$
    \item separable linear equations from \S2.2:

    $y'=g(x)y$ or $y'=ay+b$
    \item general linear equations from \S2.3: $y'+P(x)y=f(x)$                 
    \end{enumerate}
\item \S2.3 method handles all first-order \emph{linear} equations (\enumnum{3}), but other methods are usually quicker for forms \enumnum{1} or \enumnum{2}, if you have a choice
