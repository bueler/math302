...
\docdate{30 August 2023}

\begin{document}
\renewcommand{\d}{\displaystyle}

\strut
\centerline{{\Large \textsc{Some High Points of Calculus}}}

\medskip
\noindent
This is a review of some calculus you will need for differential equations.
...
\newpage
\aproblem \emph{Indefinite integration.}  Remember that the indefinite integral just means ``anti-derivative,'' so $\int f(x)\,dx = F(x) + C$ means exactly the same thing as $\left(F(x)\right)' = f(x)$.  Recall you can do some integrals just by recognizing a derivative, perhaps with some fiddling with constants.
...
\aproblem \emph{Integration by substitution.}  Substitution is \textbf{the chain rule in reverse}.  For example, from \textbf{1 (a)} we have
\begin{align*}
...
\epart{a} Turn your chain rule example in \textbf{1 (b)} into an integration by substitution.
